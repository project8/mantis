...
class MantisEgg
{
    private:
        ...
        static std::string attr_to_xml( egg_hdr_attr_k_t key, egg_hdr_attr_v_t val );
        static std::string xml_vers_header();
        static std::string xml_hdr_open();
        static std::string xml_hdr_close();
        std::size_t data_size;
        std::size_t data_width;
        bool write_raw_bytes( const void* tgt, std::size_t tgt_size, std::size_t tgt_width );
        MantisEgg();
    public:
        static MantisEgg* egg_from_env( safeEnvPtr& env );

        template< typename T1, typename T2 >
        bool add_header_attr( egg_hdr_elem_k_t key, T1 value1, T2 value2 );
        bool write_header();
        bool write_data( MantisBufferRecord* block );
        ~MantisEgg();
};
...
template< typename T1, typename T2 >
bool MantisEgg::add_header_attr( egg_hdr_elem_k_t key, T1 value1, T2 value2 )
{
