...
#ifndef MT_PARAM_JSON_HH_
#define MT_PARAM_JSON_HH_
...
namespace mantis
{
    MTLOGGER(mtlog_param_json, "param");
    ...
    bool param_output_json::write_param( const param& a_to_write, XWriter* a_writer )
    {
        ...
        MTWARN( mtlog_param_json, "parameter not written: <" << a_to_write << ">" );
        return false;
    }
    template< class XWriter >
    bool param_output_json::write_param_null( const param& /*a_to_write*/, XWriter* a_writer )
    {
        //MTWARN( mtlog_param_json, "writing null" );
        a_writer->Null();
        return true;
    }
    template< class XWriter >
    ...
    {
        //MTWARN( mtlog_param_json, "writing node" );
        ...
        for( param_node::const_iterator it = a_to_write.begin(); it != a_to_write.end(); ++it )
        {
            ...
            {
                MTERROR( mtlog_param_json, "Error while writing parameter node" );
                return false;
            }
        }
        a_writer->EndObject();
