...
namespace mantis
{
                    ...
                    f_is_loaded( false )
    {
        ...
        {
            MTWARN( mtlog, "Unable to get the username; authentications not loaded" );
        }
        ...
        if( ! t_username.empty() )
        {
            ...
            if( t_pwd_buf == NULL )
            {
                MTWARN( mtlog, "Unable to allocate the pwd buffer; authentications not loaded")
            }
            else
            {
                struct passwd t_pwd;
                ...
                {
                    MTWARN( mtlog, "Unable to get the pwd data; authentications not loaded" );
                }
                else
                {
        ...
        {
            MTDEBUG( mtlog, "Looking for authentication file: <" << t_auth_file_path << ">" );
            try
            {
                t_auth_file_present = exists( t_auth_file_path ) && is_regular_file( t_auth_file_path );
                if( ! t_auth_file_present )
                {
                    MTWARN( mtlog, "File either doesn't exist (" << exists( t_auth_file_path ) << ") or isn't a regular file (" << is_regular_file( t_auth_file_path ) << ")" );
                }
            }
            catch( filesystem_error& e )
            {
                MTWARN( mtlog, "Unable to determine if the authentication file is a regular file: " << e.what() );
            }
        }
        ...
        {
            MTWARN( mtlog, "Authentications not loaded" );
        }
    }
